...
const InfoPanel: React.FC<InfoPanelProps> = ({ node, isOpen, onNext, onClose, onOpenGuide, isLast }) => {
  ...
  return (
    <div className={`fixed top-[12%] left-0 h-[76vh] w-[33vw] min-w-[480px] bg-[#fdfaf5] shadow-[20px_0_60px_rgba(0,0,0,0.15)] transition-all duration-700 cubic-bezier(0.23, 1, 0.32, 1) z-[100] overflow-hidden ripped-right ${isOpen ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'}`}>
      ...
      <div className="relative p-8 h-full flex flex-row z-10 gap-6">
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
          ...
          <div className="flex items-center gap-4 mb-6 border-b-2 border-[#a22121]/20 pb-2 shrink-0">
            <h2 className="text-3xl shoujin-font text-[#a22121]">{node.province} · {node.location}</h2>
            <button onClick={onOpenGuide} className="flex items-center gap-2 px-3 py-1 border border-[#a22121]/30 text-[#a22121] hover:bg-[#a22121] hover:text-white transition-all rounded-sm group">
            ...
            <div className="flex flex-col h-full shrink-0 gap-8">
              {node.adventure && (
                <div className="relative py-8 px-4 bg-[#f4f1ea]/40 border-x border-[#d4af37]/15 h-fit">
                  ...
                  <div className="brush-font text-2xl text-[#1a1a1a] leading-relaxed writing-vertical italic opacity-85">“{node.adventure}”</div>
                </div>
              )}
          ...
          <div className="mt-auto pt-6 flex justify-start gap-4 shrink-0 z-20">
            <button onClick={(e) => { e.stopPropagation(); onClose(); }} className="relative flex items-center justify-center border border-[#1a1a1a]/40 py-6 px-3 group transition-all hover:bg-[#1a1a1a] hover:text-white active:scale-95 shadow-md bg-white/50 backdrop-blur-sm">
              <span className="shoujin-font text-sm writing-vertical tracking-[0.2em]">收卷暂歇</span>
            </button>
            <button onClick={(e) => { e.stopPropagation(); onNext(); }} className="relative flex items-center justify-center border border-[#9d2919] py-6 px-3 group transition-all hover:bg-[#9d2919] hover:text-white active:scale-95 shadow-lg bg-white/50 backdrop-blur-sm">
              <span className="shoujin-font text-sm writing-vertical tracking-[0.2em]">{isLast ? '掩卷合志' : '合印前行'}</span>
            </button>
          </div>
        </div>
